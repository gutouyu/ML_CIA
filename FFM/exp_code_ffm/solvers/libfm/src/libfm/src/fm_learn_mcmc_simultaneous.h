...
class fm_learn_mcmc_simultaneous : public fm_learn_mcmc {
	protected:
		...
		virtual void _learn(Data& train, Data& test) {
            ...
            //fm->show_model();
			double accumulated_training_time = 0;
			for (uint i = num_complete_iter; i < num_iter; i++) {
				...
				draw_all(train);
                accumulated_training_time += omp_get_wtime()-omp_start_time;
                ...
                double logloss_on_train = 0;
                double logloss_on_test = 0;
				...
				if (task == TASK_REGRESSION) {
					...
					for (uint c = 0; c < test.num_cases; c++) {
						double p = cache_test[c].e;
                        double y = test.target(c) > 0? 1: 0;
						p = cdf_gaussian(p);
                        double pp = std::min(std::max(1e-15, p), 1-1e-15);
                        logloss_on_test -= y*std::log(pp)+(1-y)*std::log(1-pp);
					...
					for (uint c = 0; c < train.num_cases; c++) {
						double p = cache[c].e;						
                        double y = train.target(c) > 0? 1: 0;
						p = cdf_gaussian(p);
                        double pp = std::min(std::max(1e-15, p), 1-1e-15);
                        logloss_on_train -= y*std::log(pp)+(1-y)*std::log(1-pp);
                    ...
                    std::cout << " time: " << std::setprecision(4) << std::setw(8) << accumulated_training_time;
                    std::cout << " obj: " << std::setprecision(8) << std::setw(10) << regularization+logloss_on_train
                              << " ll(train): " << std::setprecision(8) << std::setw(10) << logloss_on_train/train.num_cases
                              << " ll(test): " << std::setprecision(8) << std::setw(10) << logloss_on_test/test.num_cases << std::endl;
