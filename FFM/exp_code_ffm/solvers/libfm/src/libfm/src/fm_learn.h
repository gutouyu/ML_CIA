...
class fm_learn {
	...
	public:
        ...
        double calculate_regularization() {
            double obj = 0.5*fm->reg0*std::pow(fm->w0, 2);
            for (unsigned int j = 0; j < fm->num_attribute; j++)
                obj += 0.5*fm->regw*std::pow(fm->w.value[j], 2);
            for (unsigned int k = 0; k < fm->num_factor; k++)
                for (unsigned int j = 0; j < fm->num_attribute; j++)
                    obj += 0.5*fm->regv*std::pow(fm->v.get(k, j), 2);
            return obj;
        }
