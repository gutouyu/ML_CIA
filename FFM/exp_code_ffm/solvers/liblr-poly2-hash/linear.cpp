...
public:
	...
	static double nrm2_sq_poly2(const feature_node *x, int base, int hash_type)
	{
		double ret = 0;
		while(x->index != -1)
		{
			ret += x->value*x->value;
            if(hash_type != LINEAR)
                for(const feature_node *t = x+1; t->index != -1; t++)
                {
                    int hashed_idx = base + hash_idx(x->index, t->index, hash_type, base);
                    ret += (x->value * t->value) * (x->value * t->value);
                }
			x++;
		}
		return (ret);
	...
	static double dot_poly2(const double *s, feature_node *x, int base, int hash_type)
	{
		double ret = 0;
		while(x->index != -1)
		{
			ret += s[x->index-1]*x->value;
            if(hash_type != LINEAR)
                for(feature_node *t = x+1; t->index != -1; t++)
                {
                    int hashed_idx = base + hash_idx(x->index, t->index, hash_type, base);
                    ret += s[hashed_idx] * x->value * t->value;
                }
			x++;
		}
		return (ret);
	...
	static void axpy_poly2(const double a, feature_node *x, double *y, int base, int hash_type)
	{
		while(x->index != -1)
		{
			y[x->index-1] += a*x->value;
            if(hash_type != LINEAR)
                ...
                {
                    int hashed_idx = base + hash_idx(x->index, t->index, hash_type, base);
                    y[hashed_idx] += a * x->value * t->value;
                }
			x++;
		}
	}
...
std::unordered_map<long long, int> P2MAP;

int NUM_BINS = (1 << 23) - 1;

inline long long gen_key(int a, int b)
{
    return (((long long)a)<<32) + (long long)b;
}

inline int hash_idx(long a, long b, int hash_type, int n)
{
	switch(hash_type)
	{
		case DIRECT:
		...
		case DICT:
		{
			long long key = gen_key(a, b);
    		if(P2MAP.find(key) == P2MAP.end())
        		return 0;
    		else
        		return P2MAP.at(key);
		}
		case HASH:
		{
    		return (int)((((a+b) * (a+b+1) >> 1) + b) & NUM_BINS);
			break;
		}
	}
}

long long init_p2model(const problem *prob)
{
	for(int i=0; i<prob->l; i++)
	{
		...
		while(x->index != -1)
		{
			for(feature_node *t = x+1; t->index != -1; t++)
            {
                long long key = gen_key(x->index, t->index);
                if(P2MAP.find(key) == P2MAP.end()) {
                    int val = P2MAP.size() + 1;
                    P2MAP[key] = val;
                }
            }
            x++;
...
void l2r_lr_fun::Hv(double *s, double *Hs)
{
	...
	for(i=0;i<l;i++)
	{
		...
		wa[i] = sparse_operator::dot_poly2(s, xi, prob->n, prob->hash_type);
		
		wa[i] = C[i]*D[i]*wa[i];

		sparse_operator::axpy_poly2(wa[i], xi, Hs, prob->n, prob->hash_type);
	}
	for(i=0;i<w_size;i++)
		Hs[i] = s[i] + Hs[i];
	delete[] wa;
...
void l2r_lr_fun::Xv(double *v, double *Xv)
{
	...
	for(i=0;i<l;i++)
		Xv[i]=sparse_operator::dot_poly2(v, x[i], prob->n, prob->hash_type);
}
...
void l2r_lr_fun::XTv(double *v, double *XTv)
{
	...
	for(i=0;i<w_size;i++)
		XTv[i]=0;
	for(i=0;i<l;i++)
		sparse_operator::axpy_poly2(v[i], x[i], XTv, prob->n, prob->hash_type);
}
...
void l2r_l2_svc_fun::subXTv(double *v, double *XTv)
{
	...
	for(i=0;i<w_size;i++)
		XTv[i]=0;
	for(i=0;i<sizeI;i++)
		sparse_operator::axpy_poly2(v[i], x[I[i]], XTv, prob->n, prob->hash_type);
}
	...
	double Cp, double Cn, int solver_type)
{
	...
	for(i=0; i<l; i++)
	{
		...
		QD[i] += sparse_operator::nrm2_sq_poly2(xi, prob->n, prob->hash_type);
		sparse_operator::axpy_poly2(y[i]*alpha[i], xi, w, prob->n, prob->hash_type);
	...
	while (iter < max_iter)
	{
		...
		for (s=0; s<active_size; s++)
		{
			...
			G = yi*sparse_operator::dot_poly2(w, xi, prob->n, prob->hash_type)-1;
			...
			if(fabs(PG) > 1.0e-12)
			{
				...
				sparse_operator::axpy_poly2(d, xi, w, prob->n, prob->hash_type);
			}
		}
	...
	int solver_type)
{
	...
	for(i=0; i<l; i++)
	{
		feature_node * const xi = prob->x[i];
		QD[i] = sparse_operator::nrm2_sq_poly2(xi, prob->n, prob->hash_type);
		sparse_operator::axpy_poly2(beta[i], xi, w, prob->n, prob->hash_type);
	...
	while(iter < max_iter)
	{
		...
		for(s=0; s<active_size; s++)
		{
			...
			feature_node * const xi = prob->x[i];
			G += sparse_operator::dot_poly2(w, xi, prob->n, prob->hash_type);
...
void solve_l2r_lr_dual(const problem *prob, double *w, double eps, double Cp, double Cn)
{
	...
	{
		feature_node * const xi = prob->x[i];
		xTx[i] = sparse_operator::nrm2_sq_poly2(xi, prob->n, prob->hash_type);
		sparse_operator::axpy_poly2(y[i]*alpha[2*i], xi, w, prob->n, prob->hash_type);
		index[i] = i;
	}
	...
	while (iter < max_iter)
	{
		...
		for (s=0; s<l; s++)
		{
			...
			feature_node * const xi = prob->x[i];
			ywTx = yi*sparse_operator::dot_poly2(w, xi, prob->n, prob->hash_type);
			...
			if(inner_iter > 0) // update w
			{
				alpha[ind1] = z;
				alpha[ind2] = C-z;
				sparse_operator::axpy_poly2(sign*(z-alpha_old)*yi, xi, w, prob->n, prob->hash_type);
			}
		}
...
model* train(const problem *prob, const parameter *param)
{
    ...
    long long p2_model_size;
	if(prob->hash_type == LINEAR) 
    {
		p2_model_size = 0;
	}
    else if(prob->hash_type == DIRECT)
    {
        p2_model_size = (long long)(n+1) * (long long)(n+1);
    }
	else if(prob->hash_type == DICT)
    {
		p2_model_size = init_p2model(prob);
    }
	else if(prob->hash_type == HASH)
    {
		p2_model_size = (1LL << prob->num_bits) - 1;
    }

    if(p2_model_size >= (1LL<<31))
    {
        info("Model size too large: %d^2 = %lld\n", n+1, p2_model_size);
        exit(0);
    }
    NUM_BINS = (int)p2_model_size;
    info("num_bins = %d\n", NUM_BINS);
	...
	model_->hash_type = prob->hash_type;
	model_->num_bins = NUM_BINS;
	...
	else
	{
		...
		sub_prob.hash_type = prob->hash_type;
...
double predict_values(const struct model *model_, const struct feature_node *x, double *dec_values)
{
	...
	for(; (idx=lx->index)!=-1; lx++)
	{
        ...
        if(model_->hash_type != 0)
            for(feature_node const *mx = lx+1; mx->index != -1; mx++)
            {
                int hashed_idx = n + hash_idx(lx->index, mx->index, model_->hash_type, n);
                for(i=0;i<nr_w;i++)
                    dec_values[i] += w[hashed_idx*nr_w+i]*lx->value*mx->value;
            }
	}
...
int save_model(const char *model_file_name, const struct model *model_)
{
	...
	fprintf(fp, "bias %.16g\n", model_->bias);
	fprintf(fp, "hash_type %d\n", model_->hash_type);
	fprintf(fp, "num_bins %d\n", NUM_BINS);
...
struct model *load_model(const char *model_file_name)
{
	...
	while(1)
	{
		...
		else if(strcmp(cmd,"num_bins")==0)
		{
			FSCANF(fp,"%d",&num_bins);
			model_->num_bins=num_bins;
			NUM_BINS = num_bins;
		}
		else if(strcmp(cmd,"hash_type")==0)
		{
			FSCANF(fp,"%d",&hash_type);
			model_->hash_type=hash_type;
		}
...
const char *check_parameter(const problem *prob, const parameter *param)
{
	...
	if(param->solver_type != L2R_LR
		...
		return "Currently only support L2-regularized L1- and L2-loss SVC/SVR and L2R_LR";
