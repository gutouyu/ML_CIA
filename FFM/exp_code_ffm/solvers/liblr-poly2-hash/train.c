...
void exit_with_help()
{
	...
	"-n : num bits (for poly2 mapping, the hash space will be 2^n-1, default 23)\n"
	"-h : hash type (0 for linear, 1 for direct mapping, 2 for dictionary and 3 for hashing, default 0)\n"
	);
	exit(1);
}
...
void parse_command_line(int argc, char **argv, char *input_file_name, char *model_file_name)
{
	...
	for(i=1;i<argc;i++)
	{
		...
		switch(argv[i-1][1])
		{
			...
			case 'n':
				num_bits = atof(argv[i]);
				break;

			case 'h':
				hash_type = atoi(argv[i]);
				break;
...
void read_problem(const char *filename)
{
	...
	prob.bias=bias;
	prob.num_bits=num_bits;
	prob.hash_type = hash_type;
